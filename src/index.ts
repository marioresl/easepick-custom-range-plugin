...
export class ExtendedRangePlugin extends BasePlugin implements IPlugin {
    public tooltipElement: HTMLElement;
    public triggerElement: HTMLElement;
    private ENDLESS_DATE = new Date(3000, 0, 1, 1,0,0).toISOString();
    ...
    private onView(event: CustomEvent) {
        ...
        if(view === 'CalendarDays'){
            ...
            endButton.id = 'endless';
            endButton.className = 'day unit';
            endButton.dataset.time = this.ENDLESS_DATE;
            endButton.textContent = '∞';
            target.appendChild(endButton);

            const datePicked = this.picker.datePicked;
            const start = datePicked.length ? this.picker.datePicked[0] : this.getStartDate();
            if (endButton && start && date.getMonth() >= start.getMonth() ) {
                endButton.classList.add('in-range');
            }
        }
        ...
        if (target instanceof HTMLElement) {
            ...
            if (this.picker.isCalendarDay(element)) {
                ...
                if (this.options.tooltip) {
                    let diff = this.options.tooltipNumber(date2.diff(date1, 'day') + 1);

                    if (diff > 0) {
                        if(date1.isSame(new DateTime(this.ENDLESS_DATE)) || date2.isSame(new DateTime(this.ENDLESS_DATE))){
                            diff = Infinity;
                        }
    ...
    private onClickCalendarDay(element: HTMLElement) {
        if (this.picker.isCalendarDay(element)) {
            console.log('on click calendar day', element)
